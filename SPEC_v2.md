...
>
> **进度状态(2026-06-16 更新)**:**v2 全屏录制路线在免费签名下已真机验证成立** 🎉。
> Phase 0(socket 桥连通)、Phase 1(真实帧采集:协议分帧 + FrameSelector 筛选 + PNG 传输)、
> Phase 2(拼接接线:复用 v1 审阅/拼接/预览)代码完成、CI 绿。**背景存活难题已解决**:
> App Group 经真机确认为免费签名硬限制(正确声明 entitlement 后容器仍 nil,已彻底移除),
> 改用**静音音频后台保活**(`AudioKeepAlive` + UIBackgroundModes:audio)让主 App 切到别的
> App 录制时不被挂起,socket 持续收帧——真机验证切到微信滚动帧持续到达,不再只收前两张。
> **传输路线为本地回环 socket**(原因见 §0)。剩余为 Phase 3 调优(FrameSelector 阈值、UX、
> 状态栏/录制红条处理)与 Phase 4 收尾。
