...
>
> **进度状态(2026-06-16 更新)**:Phase 0 冒烟代码就位、CI 绿,但**真机尚未验证 socket 桥能否在
> 免费签名下连通**——这是 v2 最大未知数,仍悬而未决。Phase 1 的纯算法部分 `FrameSelector`
> 已实现并通过 CI 单测;其余采集集成(帧提取/编码/传输/拼接接线)待真机验证传输路线后再推进。
> **传输路线已从 App Group 改为本地回环 socket**(原因见 §0),下文相关章节已更新,App Group
> 方案作为历史保留在 §7。
...
### Phase 0 — 侧载冒烟测试(最高优先,先验证再投入)— 🟡 代码就位/CI 绿/真机待验
